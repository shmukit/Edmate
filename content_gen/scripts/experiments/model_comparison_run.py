from content_gen.scripts.prompts import JSON_GENERATION_PROMPT
...
def run_experiment():
    ...
    json_path = Path(
        "content_gen/data/extracted/9701_m24_qp_12 (2)_extracted.json")
    ...
    for model_cfg in models_to_test:
        ...
        output_file = Path(
            f"content_gen/data/outputs/experiment_9701_m24_{alias}.json")
        ...
        for q in questions:
            ...
            while not success and attempts < 2:
                ...
                try:
                    ...
                    required_keys = ["detailed_explanation",
                                     "option_analysis", "flashcards"]
                    if all(parsed.get(k) for k in required_keys):
                        ...
                        print(
                            f"      ⚠️ QC Failed (missing keys) for Q{q_num} on attempt {attempts}. Retrying...")
                except Exception as e:
                    print(
                        f"      ❌ Error on attempt {attempts} for Q{q_num}: {e}")
                    if attempts == 2:
