...
from scripts.processing.import_to_db import DatabaseImporter, TABLE_MAP, SUBJECT_IDS, FALLBACK_TOPIC_ID, FALLBACK_SUBTOPIC_ID
from scripts.processing.content_generator import ContentGenerator
from scripts.processing.text_normalizer import normalize, normalize_options
...
def parse_option_explanations(text: str) -> List[str]:
    ...
    for i, letter in enumerate(["A", "B", "C", "D"]):
        ...
        if match:
            ...
            content = re.sub(r"^(is|is correctly|is incorrectly|is correct|is incorrect)\s*(because|as|since|identifies)?\s*", "", match.group(1).strip(), flags=re.IGNORECASE)
    ...
    if not any(explanations):
        for i, letter in enumerate(["A", "B", "C", "D"]):
            ...
            match = re.search(pattern, text)
            if match:
                explanations[i] = normalize(match.group(1).strip(), wrap_html=False)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Generate AI content and sync to production DB")
    parser.add_argument("--paper-code", required=True, help="Paper code, e.g. 9701_w25_qp_11")
    parser.add_argument("--subject", required=True, choices=["Biology", "Chemistry", "Physics"], help="Subject")
    parser.add_argument("--grade", default="A-Level", choices=["A-Level", "IGCSE"], help="Grade")
    parser.add_argument("--provider", default="gemini", choices=["gemini", "openai", "mock"], help="LLM provider")
    parser.add_argument("--batch-size", type=int, default=1, help="Number of questions per AI call")
    parser.add_argument("--limit", type=int, default=100, help="Limit number of questions to process")
    ...
    print(f"🚀 Starting Sync for {args.paper_code} ({args.subject} {args.grade})")
    
    with DatabaseImporter(db_url) as importer:
        # 1. Fetch questions lacking content
        questions = importer.fetch_questions_for_generation(table, args.paper_code)
        
        if not questions:
            print("✅ All questions already have content. Nothing to do!")
            return
            
        print(f"📊 Found {len(questions)} questions lacking content. Processing up to {args.limit}...")
        questions = questions[:args.limit]
        
        # 2. Generate content
        gen = ContentGenerator(provider=args.provider)
        generated_results = gen.generate_for_questions(questions, args.subject, batch_size=args.batch_size)
        ...
        for q in generated_results:
            ...
            summary_exp = normalize(q.get("explanation_generated", ""), wrap_html=True)
            ...
            correct_match = re.search(r"(?i)(?:Final\s+)?Correct\s+Answer[:\s]+(?:is\s+)?([A-D])", q.get("explanation_generated", ""))
            if not correct_match:
                # Fallback: look for the letter at the end of the explanation
                correct_match = re.search(r"(?i)option\s+([A-D])\s*\.?$", q.get("explanation_generated", "").strip())
            
            # Map option letter to integer (A=0, B=1, C=2, D=3)
            option_map = {"A": 0, "B": 1, "C": 2, "D": 3}
            correct_options = [option_map[correct_match.group(1).upper()]] if correct_match else []
            ...
            if not args.dry_run:
                ...
                importer.cur.execute(f"SELECT topic_id, subtopic_id FROM {table} WHERE id = %s", (q_id,))
