...
class AutomationEngine:
    ...
    def process_pdf(self, pdf_path: str, config: Any = None, progress_callback: Optional[Callable[[float], None]] = None) -> Dict:
        """
        Processes a PDF and returns a structured dictionary (for QC Viewer compatibility).
        config can be a Dict or a string (parent_dir) for backward compatibility.
        """
        # Handle cases where config is a string (parent_dir) passed by old main.py
        if isinstance(config, (str, Path)):
            config = {}

        config = config or {}
        modalities = config.get("modalities", [
                                "core_concept", "detailed_explanation", "option_analysis", "flashcards"])
    ...
    def _extract_diagrams(self, img_bytes: bytes, questions: List[Dict]):
        ...
        try:
            ...
            for q in questions:
                ...
                coord_fields = [
                    f for f in q.keys() if f.endswith("_coords") and q[f]]
    ...
    def _call_llm(self, img_bytes: bytes, prompt: str, schema: Any) -> List[Dict]:
        ...
        if "gemini" in self.provider:
            ...
            # OpenAI uses text-based guidance for schema
            return self._call_openai(img_bytes, prompt)
        return []
    ...
    def refine_explanation(self, original_q: str, feedback: str) -> str:
        """
        Refines an explanation based on user feedback.
        """
        prompt = f"""
        ...
        """
        # Quick implementation using the existing LLM bridge
        if "gemini" in self.provider:
            response = self.client.models.generate_content(
                model=self.model_id,
                contents=prompt
