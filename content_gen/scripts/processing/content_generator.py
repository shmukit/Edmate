...
# Load prompts
try:
    from content_gen.scripts.prompts import CONTENT_GENERATION_PROMPT
except ImportError:
    try:
...
class ContentGenerator:
    ...
    def _parse_single_content(self, content: str) -> Dict:
        ...
        oe_match = re.search(
            r'(?is)\[OE_START\]\s*(.*?)\s*\[OE_END\]', content)
        ...
        if not options_body and "Option" in content:
            ...
            if len(parts) > 1:
                options_body = re.split(
                    r'(?is)\[[A-Z_]+_START\]|Concept Gap|###|---', parts[1])[0].strip()

        if not gap_body and ("Gap" in content or "Flashcards" in content):
            parts = re.split(
                r'(?is)\[GA_START\]|Concept Gap|Flashcards', content)
            if len(parts) > 1:
    ...
    def _parse_placeholders_file(self, content: str) -> List[Dict]:
        ...
        questions = []
        sections = re.split(
            r'Question\s+(\d+)Question and Options in Text Format', content)
    ...
    def _inject_content(self, original_content: str, generated_data: List[ProcessedQuestion]) -> str:
        ...
        sections = re.split(
            r'(Question\s+\d+Question and Options in Text Format)', original_content)
        header = sections[0]
            ...
            if q_num_match:
                ...
                if q_num in gen_map:
                    ...
                    q_body = q_body.replace(
                        "[EXPLANATION_PLACEHOLDER]", g.explanation_body or "[EXPLANATION_FAILED]")
                    q_body = q_body.replace(
                        "[OPTION_EXPLANATION_PLACEHOLDER]", g.option_wise_explanation or "[EXPLANATION_FAILED]")
                    q_body = q_body.replace("[FLASHCARDS_PLACEHOLDER]", "\n".join(
                        [f"{f.front_text}: {f.back_text}" for f in g.flashcards]) or "[GENERATION_FAILED]")
