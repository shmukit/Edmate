...
def parse_processed_text(file_path):
    ...
    for i in range(1, len(blocks), 2):
        ...
        detailed_exp = ""
        exp_match = re.search(r'Detailed Explanation of the Question and Right Answer(.*?)(Option Wise Explanation|### 🧠 Concept Gap Analysis)', block_content, re.DOTALL)
        if exp_match:
            detailed_exp = exp_match.group(1).strip()
            
        # 2. Extract Option Wise Explanation
        option_exp = ""
        opt_match = re.search(r'Option Wise Explanation \(Detailed\)(.*?)(### 🧠 Concept Gap Analysis|--------------------------------------------------)', block_content, re.DOTALL)
        if opt_match:
        ...
        flash_matches = re.finditer(r'Flashcard\s+\d+:\s*(.*?)\?\s*Back:\s*(.*?)\.', block_content)
    ...
    try:
        ...
        for q in results:
            ...
            cur.execute(
                ...
                WHERE id = %s
                """,
                (q['detailed_explanation'], DEFAULT_TOPIC_ID, DEFAULT_SUBTOPIC_ID, db_q_id)
            )
            
            # --- 2. Insert Flashcards ---
            cur.execute("DELETE FROM flashcards WHERE \"questionId\" = %s", (db_q_id,))
            ...
            for flash in q['flashcards']:
                ...
                cur.execute(
                    ...
                    """,
                    (card_id, flash['front'], flash['back'], DEFAULT_TOPIC_ID, DEFAULT_SUBTOPIC_ID, SUBJECT_ID, db_q_id)
                )
            
            print(f"  ✅ Imported Q{q_num}: Explanation updated, {len(q['flashcards'])} flashcards added.")
